...
class EditPermissionsPanel extends React.Component {
  ...
  loadPermissions = () => {
    ...
    this.permsRequest = 
      $.getJSON(SCRIPT_NAME_URL + `/api/projects/${this.props.projectId}/permissions/`, json => {
        let validUsernames = {};
  ...
  autocomplete = (perm) => {
      ...
      this.validateTimeout = setTimeout(() => {
        this.validateReq = $.getJSON(SCRIPT_NAME_URL + `/api/users/?limit=30&search=${encodeURIComponent(perm.username)}`)
          .done((json) => {
            json.forEach(u => {
