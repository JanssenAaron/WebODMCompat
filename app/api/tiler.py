...
def get_tile_url(task, tile_type, query_params):
    url = reverse("tiles-api-dummy-url", 
                    kwargs={
    ...
    def post(self, request, pk=None, project_pk=None, asset_type=None):
        ...
        if asset_type in ['orthophoto', 'dsm', 'dtm']:
            ...
            if export_format == 'gtiff' and (epsg == task.epsg or epsg is None) and expr is None:
                return Response({'url': reverse("download-asset-api", kwargs={"project_pk": task.project.id, "pk": task.id, "asset": asset_type+".tif" }), 'filename': filename})
            else:
            ...
            if export_format == 'laz' and (epsg == task.epsg or epsg is None):
                return Response({'url': reverse("download-asset-api", kwargs={"project_pk": task.project.id, "pk": task.id, "asset": asset_type+".laz"}), 'filename': filename})
            else:
